// Reclaim core — the part that actually touches the disk.
//
// Everything in this file runs on a worker thread (see reclaim-worker.js). It is full of
// synchronous readdir/stat calls, which is the right way to walk a filesystem and the wrong thing
// to do on Electron's main thread — doing it there freezes the window. The main process talks to
// this through agents/reclaim.js and never calls it directly.
//
// "Free up 100 GB on this machine", done properly.
//
...
// The three entry points the worker calls. Each builds its own instance: a worker is one job.
async function runScan(args = {}) {
  return new ReclaimCore().scan(args);
}

// The selection has already been checked on the main thread against the scan it came from. It is
// checked again here, because this is the code that does the deleting and it does not get to
// assume anyone upstream did their job.
async function runApply(items, onProgress) {
  const core = new ReclaimCore();
  core.last = { groups: [{ id: "selection", items: items || [] }] };
  return core.apply((items || []).map((i) => i.id), { onProgress });
}

async function runUninstall(item) {
  const core = new ReclaimCore();
  core.last = { groups: [{ id: "apps", kind: "apps", items: [item] }] };
  return core.uninstall(item.id);
}

module.exports = { ReclaimCore, runScan, runApply, runUninstall, deletable, isDenied, insideAllowed };
