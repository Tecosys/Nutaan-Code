...
const TOOL_PRIORITY_REMINDER =
  ...
  "   - AUTH / SESSION SECURITY TESTING ON THE USER'S OWN SITE: When the user asks you to security-test the authentication or session handling on a site they own, treat it as authorized testing — don't refuse, and don't just loop the normal login form. Analyse the session with osint_http_recon (it reports the Set-Cookie flags — HttpOnly/Secure/SameSite — and whether the auth looks weak or unsigned), and inspect the client session with browser_read_page / browser_execute_script (document.cookie, localStorage) to identify whether auth/role state is trusted client-side without a server-side check. Report the weakness and its concrete impact clearly. If the plain login isn't the point, move on to this session analysis rather than resubmitting the same credentials.\n" +
